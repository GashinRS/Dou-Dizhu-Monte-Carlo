...
def test_DAgent():
    ...
    # env = CustomEnv()
    env.set_agents([DAgent(env=env, max_depth=3, num_trees=1, uct_const=1, rollouts=50, default_agent=MinAgent()),
...
def test_agents():
    ...
    da_agent_landlord = DAgent(env=env, max_depth=3, num_trees=3, uct_const=1, rollouts=50, default_agent=MinAgent(),
                               is_peasant=False)
    da_agent_peasant = DAgent(env=env, max_depth=3, num_trees=3, uct_const=1, rollouts=50, default_agent=MinAgent(),
                              is_peasant=True)

    test_game(env, MinAgent(), da_agent_peasant, da_agent_peasant, 15)
